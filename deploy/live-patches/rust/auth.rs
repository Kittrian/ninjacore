...
fn read_logout_cookie_token(headers: &HeaderMap) -> Option<String> {
    let header = headers.get(header::COOKIE)?.to_str().ok()?;
    for piece in header.split(';') {
        let piece = piece.trim();
        if let Some(rest) = piece.strip_prefix("txn=") {
...
pub async fn logout(
    ...
    if let Some(token) = read_logout_cookie_token(&headers) {
    ...
    (StatusCode::OK, response_headers, Json(json!({ "success": true })))
}

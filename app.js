...
const buildCombinedAddress = ({ street = '', city = '', state = '', zip = '' } = {}) => {
  ...
  if (cityValue && stateValue) {
    localityParts.push(`${cityValue}, ${stateValue}`);
  } else if (cityValue) {
    localityParts.push(cityValue);
  } else if (stateValue) {
    localityParts.push(stateValue);
  }
  if (zipValue) {
    localityParts.push(zipValue);
  }
  const locality = localityParts.join(' ').replace(/\s+/g, ' ').trim();
...
const isTokenAgency = (agency = '') => {
  const normalized = String(agency || '').trim().toLowerCase();
  return normalized.includes('smartcredit') || normalized.includes('myfree');
};

const syncMonitoringCredentialFields = () => {
  const form = getClientForm();
  if (!form) {
    return;
  }
  const secretWrap = document.querySelector('#clientForm [data-secret-key-field]');
  const tokenWrap = document.querySelector('#clientForm [data-monitoring-token-field]');
  const secretInput = form.secretKey;
  const tokenInput = form.monitoringToken;
  const agency = String(form.monitoringAgency?.value || '').trim();
  const useToken = isTokenAgency(agency);

  if (secretWrap) {
    secretWrap.hidden = useToken;
  }
  if (tokenWrap) {
    tokenWrap.hidden = !useToken;
  }

  if (useToken && secretInput) {
    secretInput.value = '';
  }
  if (!useToken && tokenInput) {
    tokenInput.value = '';
  }
};
...
const syncDerivedClientCredentials = ({ forcePortal = false, forceSecret = false } = {}) => {
  ...
  syncMonitoringCredentialFields();
};
...
const populateAddressSuggestions = async (query) => {
  ...
  try {
    const response = await fetch(`${apiBase}/api/address-suggestions?q=${encodeURIComponent(text)}`, {
    ...
    const suggestions = Array.isArray(payload.suggestions) ? payload.suggestions.slice(0, 6) : [];
    addressSuggestionRecords = suggestions
      .map((suggestion) => {
        if (typeof suggestion === 'string') {
          const parsed = parseCombinedAddress(suggestion);
          return {
            label: suggestion,
            street: parsed.street || suggestion,
            city: parsed.city || '',
            state: parsed.state || '',
            zip: parsed.zip || '',
          };
        ...
        return {
          ...
          street: String(suggestion?.street || '').trim() || parseCombinedAddress(String(suggestion?.label || '')).street,
          city: String(suggestion?.city || '').trim() || parseCombinedAddress(String(suggestion?.label || '')).city,
          state: String(suggestion?.state || '').trim() || parseCombinedAddress(String(suggestion?.label || '')).state,
          zip: String(suggestion?.zip || '').trim() || parseCombinedAddress(String(suggestion?.label || '')).zip,
    ...
    if (hint) {
      hint.textContent = suggestions.length
        ? (payload.provider === 'google' ? 'Google-powered address suggestions are active.' : 'Address suggestions are active.')
...
const getSmartCreditIntegrationKeyForPid = () => 'smartcredit';
...
const queueClientSearchRender = () => {
  ...
  const wasTypingInClientsSearch = activeElement?.id === 'clientsTableSearchInput';
  const cursorStart = wasTypingInClientsSearch ? activeElement.selectionStart : null;
  const cursorEnd = wasTypingInClientsSearch ? activeElement.selectionEnd : null;
  ...
  searchRenderTimer = window.setTimeout(() => {
    ...
    renderClients();
    if (wasTypingInClientsSearch) {
      window.requestAnimationFrame(() => {
        const nextInput = byId('clientsTableSearchInput');
        if (!nextInput) {
          return;
        }
        nextInput.focus({ preventScroll: true });
        const start = Number.isFinite(cursorStart) ? cursorStart : nextInput.value.length;
        const end = Number.isFinite(cursorEnd) ? cursorEnd : start;
        try {
...
const normalizeClientSearchValue = (value) => String(value || '').trim().toLowerCase();
...
const getMonitoringLinkStatus = (client, agency) => {
  ...
  const integrationKey = normalizedAgency.includes('smart')
    ? getSmartCreditIntegrationKeyForPid()
  ...
  const tokenValue = String(client.monitoringToken || '').trim();
  const credsReady = Boolean(String(integration.pid || '').trim() && String(integration.tokenId || '').trim() && String(integration.apiSecret || '').trim());
...
const renderClientDetail = (client) => {
  ...
  const setAgencyButtons = (selectedAgency) => {
    ...
    monitoringLinkIndicator.title = linkStatus.linked
      ? 'Token and service credentials are ready for this client.'
      : 'Missing token or saved integration credentials for this service.';
  };
  ...
  node.querySelector('.monitoring-username-input').addEventListener('input', queueCredentialSave);
  passwordInput.addEventListener('input', queueCredentialSave);
  ...
  securityCodeInput.addEventListener('paste', (event) => {
    const pasted = event.clipboardData?.getData('text');
    if (typeof pasted !== 'string') return;
    event.preventDefault();
    securityCodeInput.value = pasted.trim();
    const activeAgency = agencyButtons.find((button) => button.classList.contains('is-active'))?.dataset.agency || 'IdentityIQ';
    if (normalizeMonitoringAgency(activeAgency).includes('identity')) {
      securityValues.identityiq = securityCodeInput.value;
    } else {
...
const renderClientDetailLoading = () => {
  ...
  shell.innerHTML = `
    <div class="detail-header detail-header-loading">
      <div class="detail-name-column">
        <p class="eyebrow">Client Detail</p>
        <div class="detail-nameplate nameplate-logo detail-nameplate-loading">
          <h3 class="detail-name detail-name-loading" data-text="\uf504 Client Detail">\uf504 Client Detail</h3>
        </div>
        <div class="detail-name-loader-row detail-name-loader-row-loading">
          <div class="nt-client-detail-codepen-loader" role="status" aria-live="polite" aria-label="Loading client detail">
            <div class="container">
              <div class="gearbox">
...
const loadClientDetail = async (clientId) => {
  ...
  try {
    const payload = await request(`/api/clients/${clientId}`);
    if (myLoadToken !== clientDetailLoadToken || state.selectedClientId !== clientId) {
      return;
    }
    ...
    byId('clientDetail').innerHTML = '<p class="muted">Failed to load client details. Please click the client again.</p>';
    setFormMessage(error.message || 'Failed to load client details.', true);
  }
};
...
const pollReportRun = async (runId, clientId, initialLogCount = 0, previousReportDate = '') => {
  ...
  while (true) {
    ...
    if (run?.status === 'completed') {
      let updatedClient = await syncSelectedClientFromServer(clientId);
      const previous = String(previousReportDate || '').trim();
      for (let attempt = 0; attempt < 6; attempt += 1) {
        if (!updatedClient) {
          break;
        }
        const current = String(updatedClient.reportDate || '').trim();
        if (!previous || (current && current !== previous)) {
          break;
        }
        await new Promise((resolve) => window.setTimeout(resolve, 700));
        updatedClient = await syncSelectedClientFromServer(clientId);
      }
      if (updatedClient) {
        updatedClient.lastReportRunStatus = 'success';
        if (state.selectedClientId === updatedClient.id) {
...
const triggerSelectedClientRefresh = async (forcePaid = false) => {
  ...
  try {
    ...
    const monitoringAgency = normalizeMonitoringAgency(selectedClient?.monitoringAgency || '');
    ...
    const previousReportDate = String(selectedClient?.reportDate || '').trim();
    if (selectedClient) {
...
const loadIntegrations = async () => {
  ...
  applyIntegrationValues('smartCreditIntegrationForm', state.integrations.smartcredit);
...
const bindEvents = () => {
  ...
  const bindTokenPasteField = (selector) => {
  ...
  bindTokenPasteField('#smartCreditIntegrationForm input[name="tokenId"]');
  bindTokenPasteField('#myFreeScoreIntegrationForm input[name="tokenId"]');
  bindTokenPasteField('#clientForm input[name="monitoringToken"]');
  ...
  byId('smartCreditIntegrationForm')?.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      state.integrations.smartcredit = payload.integration;

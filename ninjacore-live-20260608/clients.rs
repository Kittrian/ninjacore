...
pub struct ClientListItem {
    ...
    #[serde(default)] pub report_date: Option<String>,
...
fn normalize_client_record_id(id: &str) -> String {
    let trimmed = id.trim();
    let token = trimmed.rsplit_once(':').map(|(_, value)| value).unwrap_or(trimmed);
...
pub struct ListClientsQuery {
    ...
    pub status: Option<String>, // Filter by status (e.g., "client", "lead")
}
...
pub async fn list_clients(
    ...
    let clients_fut = async {
        ...
        let query_str = if let Some(ref status_filter) = params.status {
            ...
            format!(
                "SELECT id, first_name, last_name, status, phase, email, phone, updated_at, \
                 ...
                 FROM clients WHERE LOWER(status) = LOWER('{}') ORDER BY last_name, first_name",
                status_lower.replace("'", "''") // Escape single quotes
            )
        } else {
            "SELECT id, first_name, last_name, status, phase, email, phone, updated_at, \
                    next_import_mode, next_import_int, \
                    manual_next_import_start_days, manual_next_import_set_date, \
                    refresh_next_import_start_date, report_date \
             FROM clients ORDER BY last_name, first_name".to_string()
        };
        ...
        let mut items: Vec<ClientListItem> = rows
            ...
            .map(|row| {
                ...
                ClientListItem {
                    id: normalize_client_record_id(&row.id),
...
pub async fn get_client(
    ...
    let id = normalize_client_record_id(&id);
...
pub async fn get_report_history(
    ...
    let id = normalize_client_record_id(&id);
    let row = load_client_raw(&state, &id).await?;
    let record_id = row.get("id").and_then(Value::as_str).unwrap_or("").to_string();
    let client_id = row.get("client_id").and_then(Value::as_str).unwrap_or("").to_string();
    ...
    struct ReportRow {
        ...
        #[serde(default)] report_file_name: String,
        #[serde(default)] response_url: String,
        #[serde(default)] created_at: String,
        #[serde(default)] report_json: String,
    }

    let mut resp = state
        .db
        .query(
            "SELECT id, client_id, source, monitoring_agency, report_date, report_file_name, response_url, created_at, report_json FROM reports WHERE client_id = $cid0 OR client_id = $cid1 OR client_id = $cid2 ORDER BY created_at DESC LIMIT 25",
        )
        .bind(("cid0", id.clone()))
        .bind(("cid1", client_id.clone()))
        .bind(("cid2", record_id.clone()))
        .await?;
    let rows = crate::db::take_many::<ReportRow>(&mut resp, 0)?;
    let history: Vec<ReportHistoryItem> = rows
        .into_iter()
        .map(|row| ReportHistoryItem {
            id: row.id,
    ...
    Ok(Json(json!({ "clientId": id, "history": history })))
}
...
pub async fn delete_client(
    ...
    let mut __resp = state
        ...
        .bind(("id", normalize_client_record_id(&id)))
        .await?;
...
fn client_to_camel(row: &Value) -> Value {
    let id_str = row
        .get("id")
        .and_then(Value::as_str)
        .map(normalize_client_record_id)
        .unwrap_or_default();
...
fn to_safe_client(mut v: Value) -> Value {
    if let Some(obj) = v.as_object_mut() {
        ...
        if let Some(id) = obj.get("id").and_then(|value| value.as_str()) {
            obj.insert("id".into(), Value::String(normalize_client_record_id(id)));
        }
        if let Some(client_id) = obj.get("client_id").cloned() {
            obj.insert("clientId".into(), client_id);
        }
        if let Some(report_date) = obj.get("report_date").cloned() {
            obj.insert("reportDate".into(), report_date);
        }

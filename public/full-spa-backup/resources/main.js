    ...
    var gettingTransunionReport = async function () {
        ...
        if (content) {
            try {
                ...
                console.log({ data })
                $scope.TUCVantageScore = $scope.EXPVantageScore = $scope.EQFVantageScore = undefined;
                if (data && data.BundleComponents && data.BundleComponents.BundleComponent) {
                    $.each($scope.makeArray(data.BundleComponents.BundleComponent), function (index, component) {
                        console.log({ index, component })
                        if (component.TrueLinkCreditReportType) {
                            $scope.reports = component.TrueLinkCreditReportType;
                        ...
                        console.log({ $scope , tradeLinePartitions})
                        if (component.CreditScoreType && component.Type) {
                            ...
                            if (component.CreditScoreType["@scoreName"] == 'VantageScore3' && component.CreditScoreType.Source.Bureau["@symbol"] == 'EXP')
                                $scope.EXPVantageScore = component.CreditScoreType;
                            if (component.CreditScoreType["@scoreName"] == 'VantageScore3' && component.CreditScoreType.Source.Bureau["@symbol"] == 'EQF')
                                $scope.EQFVantageScore = component.CreditScoreType;

...
const setAppVisibility = (loggedIn) => {
  const appNode = byId('paymentsApp');
  if (appNode) {
    appNode.hidden = !loggedIn;
  }
};
  ...
  byId('paymentsLogoutButton').addEventListener('click', () => {
    void (async () => {
      try {
        ...
        appendLog(`Logout request failed: ${error?.message || 'Unknown error'}`);
      }
...
const init = async () => {
  ...
  try {
    const statusResponse = await fetch('/api/auth/status', { method: 'GET' });
    const statusPayload = await statusResponse.json().catch(() => ({}));
    if (!statusResponse.ok || !statusPayload.authenticated) {
      window.location.replace('/');
      return;
    }
    state.ownerKey = String(statusPayload.user || getCookieValue('txn') || '').trim().toLowerCase();
  } catch {
    window.location.replace('/');
    return;
  }
